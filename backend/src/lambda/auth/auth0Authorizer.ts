...
const jwksUrl = 'https://dev-vzp0zhczidw26ys0.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  logger.info('signKeys', signKeys)
  if (!signKeys){
    throw new Error('The JWKS endpoint did not contain any Keys')
  }

  //get now the pem data for the certificate
  const pemData = signKeys.x5c[0]

  //convert to look like certificate
